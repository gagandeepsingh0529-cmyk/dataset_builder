...
import { Activity, BarChart3, Database, Factory, MapPin, Play, RotateCcw, Search, Sparkles, Square } from 'lucide-react';
...
export function Dashboard() {
  ...
  const metrics = useQuery({ queryKey: ['model-metrics'], queryFn: apiClient.modelMetrics });
  ...
  return (
    <main className="app-shell">
      ...
      <section className="hero-row">
        ...
        <div className="hero-actions"><button className="demo-button" onClick={runDemo} disabled={demoRunning || mapInventory.isLoading} aria-label="Run a demo using a real inventory record">{demoRunning ? <Square size={14} /> : <Play size={14} />}{demoRunning ? 'RUNNING REAL DEMO' : 'RUN REAL DEMO'}</button><div className="hero-stat"><Activity size={17} /><span>MODEL STATUS</span><strong>{metrics.data?.model ?? 'Awaiting model'}</strong></div></div>
      </section>
...
function ModelMetrics({ metrics, loading }: { metrics?: { model: string; r2: number; mae: number; rmse: number; training_rows?: number; test_rows?: number; training_r2?: number; cross_validation_r2_mean?: number; grouped_cross_validation_r2_mean?: number | null }; loading: boolean }) {
  if (loading) return <LoadingBlock />; if (!metrics) return <ErrorState message="Model metrics unavailable." />;
  return <div className="model-metrics"><p className="eyebrow">MODEL PERFORMANCE / HOLDOUT</p><h3>{metrics.model}</h3><div className="r2-score">{(metrics.r2 * 100).toFixed(1)}<small>% R²</small></div><div className="metric-pair"><span>MAE <b>{tonnes(metrics.mae)} tonnes</b></span><span>RMSE <b>{tonnes(metrics.rmse)} tonnes</b></span><span>Evaluation <b>{metrics.test_rows ?? '—'} test / {metrics.training_rows ?? '—'} total</b></span><span>5-fold validation <b>{metrics.cross_validation_r2_mean == null ? '—' : `${(metrics.cross_validation_r2_mean * 100).toFixed(1)}% R²`}</b></span><span>Train fit diagnostic <b>{metrics.training_r2 == null ? '—' : `${(metrics.training_r2 * 100).toFixed(1)}% R²`}</b></span></div><p className="model-note">The holdout and cross-validation scores measure generalization. The train-fit value is intentionally shown only as an overfit diagnostic.</p></div>;
}
